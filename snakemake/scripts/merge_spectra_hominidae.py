...
def get_sfs(file: str) -> sf.Spectrum:
    """
    Get SFS from file.
    ...
    :return: The SFS.
    """
    return sf.Spectrum(pd.read_csv(file, header=None).iloc[0].tolist())


neutral = sf.Spectra.from_spectra(dict((t, get_sfs(sfs_neutral[i])) for i, t in enumerate(types)))
selected = sf.Spectra.from_spectra(dict((t, get_sfs(sfs_selected[i])) for i, t in enumerate(types)))
