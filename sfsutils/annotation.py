...
class Annotation(ABC):
    ...
    @abstractmethod
    def annotate_site(self, variant: Site):
        """
...
class DegeneracyAnnotation(Annotation):
    ...
    def _parse_codon_backward(self, variant: Site):
        """
    ...
    def _parse_codon(self, variant: Site):
        """
...
class SynonymyAnnotation(DegeneracyAnnotation):
    ...
    .. warning::
        Not recommended for use with :class:`~sfsutils.parser.Parser` as we also need to annotate mono-allelic sites.
        Consider using :class:`~sfsutils.annotation.DegeneracyAnnotation` and
        :class:`~sfsutils.parser.DegeneracyStratification` instead.
    """
    ...
    def _get_alt_allele(self, variant: Site) -> str | None:
        """
    ...
    def _parse_codons_vep(self, variant: Site) -> List[str]:
        """
    ...
    @staticmethod
    def _parse_synonymy_snpeff(variant: Site) -> int | None:
        """
...
class MaximumParsimonyAncestralAnnotation(AncestralAlleleAnnotation):
    ...
    :class:`~sfsutils.parser.AncestralBaseStratification` and :class:`Annotator` or :class:`~sfsutils.parser.Parser`.
    ...
    spectra altogether. Alternatively, you can use :class:`~sfsutils.filtration.DeviantOutgroupFiltration` to
...
class AdaptivePolarizationPrior(PolarizationPrior):
    ...
    frequency counts by calling :meth:`~sfsutils.annotation.PolarizationPrior.plot`. If they are not
    smooth enough, you can increase the number of sites, decrease the number of ingroups, or
    use :class:`~sfsutils.annotation.KingmanPolarizationPrior` instead.
...
class MaximumLikelihoodAncestralAnnotation(_OutgroupAncestralAlleleAnnotation):
    ...
    ancestral allele. To be used with :class:`Annotator` or :class:`~sfsutils.parser.Parser`.
    ...
    be used by :class:`~sfsutils.parser.Parser` to polarize the SFS according to the ancestral allele probability.
    ...
    .. note::
        ...
        * The model determines the probability of the major allele being ancestral opposed to the minor allele. This can
          ...
          ingroups (see :attr:`PolarizationPrior.allow_divergence`). That said, if only the SFS is of interest,
          allow_divergence should not be set to ``True``.
    ...
    def _from_vcf(
        ...
        :class:`~sfsutils.parser.Parser` classes instead.
