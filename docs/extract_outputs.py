...
Each section of a page has a directory ``<number>-<heading>``, numbered in page order from ``00`` for the part before
the first subheading, holding the ``python-<n>`` or ``r-<n>`` files of the notebook's language in display order: figures
as PDF, or PNG without a PDF rendering, and other output as text. Since the notebooks of both languages hold the same
headings, their files share the section directories, and extracting one language keeps the files of the other. The
metadata of a PDF that differs between renderings of the same figure, the timings of progress bars and the paths of
temporary files are replaced by fixed values.

The Snakemake rule ``extract_page_outputs`` writes the outputs of
``snakemake/results/docs/{language}/{page}.executed.ipynb``. Run directly, ``python docs/extract_outputs.py <notebook>
...`` does the same for each executed notebook.
"""
import base64
import json
import re
...
def slug(title: str) -> str:
    """Directory name of a section heading: its words in lower case joined by hyphens, without MyST roles and literals."""
...
def output_file(data: dict) -> tuple[str, bytes]:
    ...
    or the text without ANSI colour codes, trailing spaces, progress bar timings and the paths of temporary files.
    """
