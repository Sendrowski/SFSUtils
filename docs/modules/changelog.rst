...
- Class-resolved :meth:`~sfsutils.spectrum.TwoSFS.covariance` and :meth:`~sfsutils.spectrum.TwoSFS.correlation` matrices of the derived-allele-count classes between two linked polymorphic sites, computed from the segregating interior (:meth:`~sfsutils.spectrum.TwoSFS.interior`) and therefore independent of the monomorphic sites and of any target-site count.
