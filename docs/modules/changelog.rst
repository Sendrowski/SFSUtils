...
- Class-resolved :meth:`~sfsutils.spectrum.TwoSFS.covariance` and :meth:`~sfsutils.spectrum.TwoSFS.correlation` of the derived-allele-count classes between two linked polymorphic sites, each returned as a :class:`~sfsutils.spectrum.TwoSFS` over the segregating interior (:meth:`~sfsutils.spectrum.TwoSFS.interior`) and therefore independent of the monomorphic sites and of any target-site count.
