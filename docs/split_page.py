...
notebook opens with a hidden setup cell common to all pages, which renders figures as PNG and PDF at the given resolution.

The Snakemake rule ``split_page`` writes ``snakemake/results/docs/Python/{name}.ipynb`` and
``snakemake/results/docs/R/{name}.ipynb``, with the resolution ``DOCS_FIGURE_DPI`` of the Snakefile. Run directly, ``python docs/split_page.py <dpi> <name> ...``
...
KERNELS = {
    ...
    "r": dict(kernelspec=dict(name="ir-sfsutils", display_name="R (sfsutils)", language="R"),
...
if __name__ == "__main__":
    try:
        ...
        dpi = int(sys.argv[1])
        results = ROOT / "snakemake" / "results" / "docs"
        jobs = [(ROOT / "docs" / "source" / f"{name}.md", results / "Python" / f"{name}.ipynb",
                 results / "R" / f"{name}.ipynb") for name in sys.argv[2:]]
